...
*Gameobject Detection on Click* - The player can click on a gameobject and the game will recognize what gameobject that the player is clicking on. This is done using a raycast at the mouse’s position which hits the gameobject’s collider and returns the gameobject. This functionality works the same with touch controls. Also the game will not detect gameobjects if the player is clicking on the UI so that players won’t do something like accidentally having an ant use up resources to build something when they’re actually trying to buy something in the shop.
